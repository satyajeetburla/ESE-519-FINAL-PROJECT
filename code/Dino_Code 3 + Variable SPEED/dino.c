...
//All GPIOs/events added in this way on the same core share the same callback; for multiple independent handlers for different GPIOs you should use gpio_add_raw_irq_handler and related functions.
    ...
    sleep_ms(10000); //Sleep for 10 seconds to let the player sit down, have a drink and get ready
    ...
    writeString("Greetings! Team - Penn Dinosaours!"); 
    ...
    while(1){ //While eternity
        ...
        unsigned long begin_time = (unsigned long)(get_absolute_time()._private_us_since_boot / 1000); //Save the start time of the loop
        ...
        if(resetGame){ //If the reset flag is set
            ...
            writeString("Well Played! Better Luck Next Time !"); 
        ...
        if(myPlayer.alive){ //If the player is alive
            ...
            if((myPlayer.x + myPlayer.w > obstacle.x) && (myPlayer.x < obstacle.x + obstacle.w) && (myPlayer.y < obstacle.h)){ //If the player has hit the obstacle
                ...
                setCursor(100, 200); //Set the cursor
                writeString("Well Played! Better Luck Next Time !"); //Print the message
                sleep_ms(100);
            }
        }
        ...
        //double exTime1 = 33 - executionTime;
        exTime = 33 - ((unsigned long)(get_absolute_time()._private_us_since_boot / 1000) - begin_time); //Calculate the amount of time to sleep for to achieve 30fps
        // printf("%u\n", exTime); //Print the time out
        //printf("%u\n", exTime1); //Print the time out
